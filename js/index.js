...
let BuyCDBtn = $("#BuyCDBtn");
let contractExpiredBtn = $("#contractExpiredBtn");
let terminateContractBtn = $("#terminateContractBtn");
let contractValue = $("#contractValue")
let contractPeriods = $("#contractPeriods")
let hasExpiredPeriods = $("#hasExpiredPeriods")
...
/////////////////////////////////////////
BuyCDBtn.on("click", async function() {
    ...
    //買定存
        ...
        bank.methods
        .buyContractDeposit(parseInt(contractValue.val(), 10),parseInt(contractPeriods.val(), 10))
        .send({
            from: nowAccount,
            gas: 3400000
            })
        ...
        .on("receipt", function(receipt) {
            log(receipt.events.buyContractDepositEvent.returnValues, "購買定存成功");
            // 觸發更新帳戶資料
            update.trigger("click");
            $("#cValue").text("金額: " + contractValue.val());
            $("#cPeriods").text("期數: " + contractPeriods.val());
            // 更新介面
            doneTransactionStatus();
        })
    .on("error", function(error) {
...
contractExpiredBtn.on("click", async function() {
    if (bankAddress == "") {
    return;
    }
...
terminateContractBtn.on("click", async function() {
    if (bankAddress == "") {
    return;
    }
    ...
    waitTransactionStatus();
    bank.methods
    .terminateContract( parseInt(hasExpiredPeriods.val(), 10))
    .send({
        from: nowAccount,
        gas: 3400000
    })
